...
class Scanner {
  ...
  async request(bacnetClient, uuid) {

    const self = this;
    bacnetClient.whoIs();
    scanArray[0].children = [];
    ...
    function getAllPropertiesManually(address, objectId, callback, propList, result) {
      ...
      bacnetClient.readProperty(address, objectId, prop, (err, value) => {
        if (!err) {
          if (debug) {
            this.plugin.log('Handle value ' + prop + ': ', JSON.stringify(value),2);
          }
          //self.plugin.log('Handle value ' + prop + ': ' + JSON.stringify(value));
          const objRes = value.property;
          objRes.value = value.values;
          result.push(objRes);
        } else {
          //self.plugin.log('Device do not contain object ' + Bacnet.enum.getEnumName(Bacnet.enum.PropertyIdentifier, prop), 2);
        }
    ...
    function parseValue(address, objId, parentType, value, supportsMultiple, callback) {
      let resValue = null;
      //self.plugin.log("value type " + Bacnet.enum.getEnumName(Bacnet.enum.ApplicationTag, value.type))
      //self.plugin.log("value type: " + Object.keys(Bacnet.enum.ApplicationTag).find(key => Bacnet.enum.ApplicationTag[key] === value.type));
      if (value && value.type && value.value !== null && value.value !== undefined) {
        switch (value.type) {
          ...
          case Bacnet.enum.ApplicationTag.BOOLEAN:
            ...
            resValue = !!value.value +':'+Bacnet.enum.getEnumName(Bacnet.enum.ApplicationTag, value.type);
            break;
          ...
          case Bacnet.enum.ApplicationTag.BIT_STRING:
            ...
            if (ObjectTypeSpecificPropertyIdentifierToEnumMap[parentType] && ObjectTypeSpecificPropertyIdentifierToEnumMap[parentType][objId]) {
              ...
              if (parentType !== Bacnet.enum.ObjectType.BITSTRING_VALUE) {
                self.plugin.log('Unknown value for BIT_STRING type for objId ' + Bacnet.enum.getEnumName(Bacnet.enum.PropertyIdentifier, objId) + ' and parent type ' + Bacnet.enum.getEnumName(Bacnet.enum.ObjectType, parentType), 2);
              }
              resValue = value.value +':'+Bacnet.enum.getEnumName(Bacnet.enum.ApplicationTag, value.type);
            }
            break;
          ...
          case Bacnet.enum.ApplicationTag.READ_ACCESS_RESULT: // ????
            ...
            break;
          default:
            self.plugin.log('unknown type ' + value.type + ': ' + JSON.stringify(value), 2);
            resValue = value;
        }
      }

      setImmediate(() => callback(resValue, value.type));
    }
      ...
      obj.values.forEach(devBaseObj => {
        ...
        devBaseObj.values.forEach(devObj => {
          ...
          devObj.value.forEach(val => {
            ...
            cbCount++;
            parseValue(address, devObj.id, parent.type, val, supportsMultiple, (parsedValue, valueType) => {
              if (debug) {
                console.log('RETURN parsedValue', deviceId, objId, devObj.value, parsedValue, Bacnet.enum.getEnumName(Bacnet.enum.ApplicationTag, valueType));
              }
              //self.plugin.log('RETURN parsedValue' + deviceId + objId + devObj.value + parsedValue + Bacnet.enum.getEnumName(Bacnet.enum.ApplicationTag, valueType));

              objDef[deviceId][objId] = objDef[deviceId][objId] || [];
              objDef[deviceId][objId].push(parsedValue);
            //  if (valueType != undefined) {
             //   objDef[deviceId][objId].push(Bacnet.enum.getEnumName(Bacnet.enum.ApplicationTag, valueType));
             // }
              if (!--cbCount) {
                finalize();
              }
     ...
     * @param obj
     */
    function printResultObject(deviceId, obj, address) {
      objectsDone++;
      let children = [];
      //console.log(`Device ${deviceId} (${objectsDone}/${Object.keys(knownDevices).length}) read successfully ...`);
      //self.plugin.log(JSON.stringify(obj, null, 2));
        obj["OBJECT_LIST(76)"].forEach(item => {
          if (item["OBJECT_IDENTIFIER(75)"] !== undefined) {
            let id = {};
            id.objectId = item["OBJECT_IDENTIFIER(75)"];
            id.address = address.address;
            const name = item["OBJECT_NAME(77)"] ? item["OBJECT_NAME(77)"].split(':')[0] : 'unknown';
            const dataType = item["PRESENT_VALUE(85)"] ? item["PRESENT_VALUE(85)"].split(':')[1] : 'unknown'
            children.push({
              id: JSON.stringify(id), 
              title: name +  ' (' + item["OBJECT_TYPE(79)"] + ') ' + item["PRESENT_VALUE(85)"] + ' (STATUS: ' + item["STATUS_FLAGS(111)"] + ') ',
              channel: {
                topic: item["OBJECT_NAME(77)"], 
                chan: JSON.stringify(id), 
                title: name, 
                objectType: item["OBJECT_TYPE(79)"],
                dataType: item["OBJECT_TYPE(79)"] == 'BINARY_VALUE(5)' ? 'ENUMERATED(9)' : dataType, 
                statusCode: item["STATUS_FLAGS(111)"], 
      ...
      scanArray[0].children.push({
        id: deviceId,
        title: JSON.stringify(address),
        children
      });

      self.plugin.log('SEND SCAN TREE for ' + util.inspect(scanArray, null, 6), 2);
      self.clients.forEach(uuid => self.plugin.send({ type: 'scan', op: 'list', data: scanArray, uuid }))
    ...
    bacnetClient.on('iAm', (device) => {
      ...
      self.plugin.log('Found Device ' + deviceId + ' on ' + JSON.stringify(address), 2);
      ...
      //self.plugin.log("knownDevices: " + knownDevices);
      bacnetClient.readPropertyMultiple(address, requestArray, (err, value) => {
        if (err) {
          self.plugin.log(deviceId + 'No ReadPropertyMultiple supported:' + err.message, 2);
          getAllPropertiesManually(address, {type: 8, instance: deviceId}, result => {
            parseDeviceObject(address, result, {type: 8, instance: deviceId}, false, res => printResultObject(deviceId, res, address));
          });
        } else {
          self.plugin.log(deviceId + 'ReadPropertyMultiple supported ...', 2);
          getAllPropertiesManually(address, {type: 8, instance: deviceId}, result => {            
            parseDeviceObject(address, result, {type: 8, instance: deviceId}, false, res => {
              printResultObject(deviceId, res, address)
            });
          });
         //parseDeviceObject(address, value, {type: 8, instance: deviceId}, true, res => printResultObject(deviceId, res, address));
        }
      });
    });
  }
  ...
  sendTree() {
    this.plugin.log('SEND SCAN TREE for ' + util.inspect(scanArray), 2);
    this.client.forEach(uuid => this.plugin.send({ type: 'scan', op: 'list', data: scanArray, uuid }))
