...
class Scanner {
  ...
  async request(bacnetClient, uuid) {
      ...
      if (value && value.type && value.value !== null && value.value !== undefined) {
        switch (value.type) {
          ...
          case Bacnet.enum.ApplicationTag.BIT_STRING:
            ...
            if (ObjectTypeSpecificPropertyIdentifierToEnumMap[parentType] && ObjectTypeSpecificPropertyIdentifierToEnumMap[parentType][objId]) {
              ...
              if (parentType !== Bacnet.enum.ObjectType.BITSTRING_VALUE) {
                self.plugin.log('Unknown value for BIT_STRING type for objId ' + Bacnet.enum.getEnumName(Bacnet.enum.PropertyIdentifier, objId) + ' and parent type ' + Bacnet.enum.getEnumName(Bacnet.enum.ObjectType, parentType));
              }
              resValue = value.value +':'+Bacnet.enum.getEnumName(Bacnet.enum.ApplicationTag, value.type);
            }
            break;
          ...
          case Bacnet.enum.ApplicationTag.READ_ACCESS_RESULT: // ????
            ...
            break;
          default:
            self.plugin.log('unknown type ' + value.type + ': ' + JSON.stringify(value));
            resValue = value;
        }
      }
    ...
    function printResultObject(deviceId, obj, address) {
      ...
      obj["OBJECT_LIST(76)"].forEach(item => {
        if (item["OBJECT_IDENTIFIER(75)"] !== undefined) {
          ...
          children.push({
            ...
            title: item["OBJECT_NAME(77)"].split(':')[0] +  ' (' + item["OBJECT_TYPE(79)"] + ') ' + item["PRESENT_VALUE(85)"] + ' (STATUS: ' + item["STATUS_FLAGS(111)"] + ') ',
            channel: {topic: item["OBJECT_NAME(77)"], 
              chan: JSON.stringify(id), 
              title: item["OBJECT_NAME(77)"].split(':')[0], 
              objectType:item["OBJECT_TYPE(79)"],
              dataType: item["OBJECT_TYPE(79)"] == 'BINARY_VALUE(5)' ? 'ENUMERATED(9)' :item["PRESENT_VALUE(85)"].split(':')[1], 
      ...
      self.plugin.log('SEND SCAN TREE for ' + util.inspect(scanArray, null, 6));
    ...
    bacnetClient.on('iAm', (device) => {
      ...
      this.plugin.log('Found Device ' + deviceId + ' on ' + JSON.stringify(address));
  ...
  sendTree() {
    this.plugin.log('SEND SCAN TREE for ' + util.inspect(scanArray));
